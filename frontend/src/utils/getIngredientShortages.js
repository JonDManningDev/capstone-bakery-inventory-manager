...
export function getIngredientShortages(recipe, ingredients, conversions, setShortages) {
    const inventoryMap = new Map(ingredients.map((ingredient) => [ingredient.ingredient_id, ingredient]));
    const shortages = [];

    for (const ingredient of recipe.ingredients) {
      const inStock = inventoryMap.get(ingredient.ingredient_id);
      ...
      const convertedAmount = convertUnits(amount_needed, unit, baseUnit, conversions);
